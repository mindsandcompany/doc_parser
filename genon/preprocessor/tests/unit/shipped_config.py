"""출고 custom_fields 설정을 내부(v1) 형태로 읽는 테스트 공용 헬퍼.

출고 설정(`resource/`, `resource_dev/`)은 v2 표기다. `yaml.safe_load` 로 raw 를 읽고
v1 최상위 키(`text_fields`, `field_labels`, `body_fields`, `chunk_prefix_fields`,
`first_chunk_fields` …)를 찾으면 전부 None 이 되어 **검사가 조용히 무력해진다** —
통과하지만 아무것도 보지 않는 상태가 된다. 실제로 그 상태로 4개 파일이 흘러갔다.

매퍼가 하는 것과 같은 번역(`config_v2.load`)을 거쳐 한 모양으로 맞춘다. 검사마다 v2 키를
