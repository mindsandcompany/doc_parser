...
from typing import Optional, Iterable, Any, List, Dict, Tuple, Iterator, Union
...
def process_chunks(chunks: list):
    new_chunk = []
    chunks = chunks

    if len(chunks)>1:
        for idx, chunk in enumerate(chunks):
            if '친환경으로 미래를 여는 에너지 전문기업' in chunk.text and chunk.meta.doc_items[0].prov[0].page_no < 2:
                continue
            if '친환경으로 미래를 여는 에너지 전문기업' in chunk.text:
                continue
            if '미래 가치를 선도하는 종합에너지 기업' in chunk.text:
                continue
            if chunk.meta.doc_items[0].label == 'table' and chunk.meta.doc_items[0].data.table_cells[0].text in ['직원','주임','대리','과장','차장']:
                continue
...
class DocumentProcessor:
    ...
    def __init__(self):
        ...
        # pipe_line_options.ocr_options.model_storage_directory = "/home/mnc/.EasyOCR/model"
