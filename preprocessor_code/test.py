...
from docling.datamodel.pipeline_options import (
    ...
    # PdfBackend,
    PdfPipelineOptions,
    TableFormerMode, RapidOcrOptions, TesseractOcrOptions
)
# docling imports
...
def update_delete_line_from_text(header_table_delete_line, result, page_delete_lines, max_height):
    ...
    for item, level in result.iterate_items():
        ...
        if isinstance(item, TextItem):
            page_no = item.prov[0].page_no
            if page_no in page_delete_lines_text and item.prov[0].bbox.t > page_delete_lines_text[page_no] and \
                    item.prov[0].bbox.b < page_delete_lines_text[page_no]:
                if page_no in page_delete_lines:
...
def process_pdf(org_document: DoclingDocument):
    ...
    updated_page_delete_lines = update_delete_line_from_text(header_table_delete_line, org_document, page_delete_lines,
                                                             max_height)
...
class GenOSVectorMetaBuilder:
    def __init__(self):
        ...
        self.chunk_bboxes: str = None
        self.media_files: str = None
...
class DocumentProcessor:
    ...
    def __init__(self):
        ...
        rapidOption = RapidOcrOptions(force_full_page_ocr=True)
        rapidOption.lang = ["korean", "english"]
        rapidOption.rec_model_path = "/nfs-root/aiModel/.RapidOCR/korean_PP-OCRv3_rec_infer.onnx"
        rapidOption.rec_keys_path = "/nfs-root/aiModel/.RapidOCR/korean_dict.txt"
        rapidOption.det_model_path = "/nfs-root/aiModel/.RapidOCR/Multilingual_PP-OCRv3_det_infer.onnx"
        easyOption = EasyOcrOptions(force_full_page_ocr=True)
        easyOption.lang = ["ko", "en"]
        easyOption.model_storage_directory = "/nfs-root/aiModel/.EasyOCR/model"
        easyOption.use_gpu = True
        tesserOption = TesseractOcrOptions(force_full_page_ocr=True)
        tesserOption.lang = ["kor", "eng"]
        tesserOption.path = "/usr/share/tessdata/"

        # `use_gpu and accelerator_options.device == AcceleratorDevice.CUDA` the GPU is used to run EasyOCR. Otherwise, EasyOCR runs in CPU.
        # pipe_line_options.ocr_options = rapidOption
        pipe_line_options.ocr_options = tesserOption

        # pipe_line_options.ocr_options.det_use_cuda = True
        # pipe_line_options.ocr_options.det_use_cuda = True
        # pipe_line_options.ocr_options.rec_use_cuda = True
