    ...
    if len(chunks) > 1:
        for idx, chunk in enumerate(chunks):
            ...
            if '미래 가치를 선도하는 종합에너지 기업' in chunk.text:
                continue
            if chunk.meta.doc_items[0].label == 'table' and chunk.meta.doc_items[0].data.table_cells[0].text in ['직원',
                                                                                                                 '주임',
                                                                                                                 '대리',
                                                                                                                 '과장',
                                                                                                                 '차장']:
...
class DocumentProcessor:
    ...
    def split_documents(self, documents: DoclingDocument, **kwargs: dict) -> list[DocChunk]:
        ...
        """
        max_char_len = kwargs.get("max_char_len", 512)
        chunker = HybridChunker()
        raw_chunks: list[DocChunk] = list(chunker.chunk(dl_doc=documents, **kwargs))
        #raw_chunks = process_chunks(raw_chunks)
