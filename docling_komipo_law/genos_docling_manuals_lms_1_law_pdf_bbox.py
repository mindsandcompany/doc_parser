...
class GenOSVectorMetaBuilder:
    def __init__(self):
        ...
        self.page_bboxes: list = None
        self.media_filepath: list = None
    ...
    def set_page_bboxes(self, doc_items: list) -> "GenOSVectorMetaBuilder":
        self.page_bboxes = []
        for item in doc_items:
            page_no = item.prov[0].page_no
            bbox = item.prov[0].bbox
            bbox_data = {'l':bbox.l,'t':bbox.t,'r':bbox.r,'b':bbox.b}
            self.page_bboxes.append({'page':page_no,'bbox':bbox_data})
        return self

    def set_media_filepath(self, doc_items: list) -> "GenOSVectorMetaBuilder":
        temp_list = []
        for item in doc_items:
            if isinstance(item, PictureItem):
                temp_list.append(str(item.image.uri))
        self.media_filepath = temp_list
        return self
    ...
    def compose_vectors(self, document: DoclingDocument, chunks: List[DocChunk], file_path: str, **kwargs: dict) -> \
        ...
        for chunk_idx, chunk in enumerate(chunks):
            ...
            vector = (GenOSVectorMetaBuilder()
                      ...
                      .set_page_bboxes(chunk.meta.doc_items)
                      .set_media_filepath(chunk.meta.doc_items)
                      ).build()
